...
contains
 ...
 !> Call procedure for each box in tree, with integer argument iarg
  subroutine af_loop_box_iarg(tree, my_procedure, iarg, leaves_only)
    type(af_t), intent(inout)     :: tree
    procedure(af_subr_iarg)        :: my_procedure
    ...
    do lvl = 1, tree%highest_lvl
       if (leaves) then
          ...
          do i = 1, size(tree%lvls(lvl)%leaves)
             id = tree%lvls(lvl)%leaves(i)
             call my_procedure(tree%boxes(id), iarg)
          end do
          !$omp end do
       else
          !$omp do
    ...
    !$omp end parallel
  end subroutine af_loop_box_iarg
