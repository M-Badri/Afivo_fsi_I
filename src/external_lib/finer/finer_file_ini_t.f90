...
type :: t_config
  ...
  contains
    ...
    procedure, pass(self) :: build                                      !< Load file data.
    generic               :: loop         => loop_options_section, &    !< Loop over options of a section.
                                             loop_options               !< Loop over all options.
    procedure, pass(self) :: print        => print_t_config             !< Pretty printing data.
    procedure, pass(self) :: save         => save_t_config              !< Save data.
    procedure, pass(self) :: section      => section_t_config           !< Get section name once provided an index.
    ! operators overloading
    generic :: assignment(=) => assign_t_config !< Procedure for section assignment overloading.
    ! private methods
    ...
    ! assignments
    procedure, private, pass(lhs) :: assign_t_config !< Assignment overloading.
endtype t_config
...
contains
  ...
  class(t_config),        intent(in) :: self         !< File data.
  ...
  !< Free dynamic memory.
  class(t_config), intent(inout) :: self !< File data.
  ...
  subroutine build(self, separator, filename, source, error)
  ...
  subroutine print_t_config(self, unit, pref, retain_comments, iostat, iomsg)
  ...
  endsubroutine print_t_config

  subroutine save_t_config(self, retain_comments, iostat, iomsg, filename)
  !< Save data.
  class(t_config),        intent(inout) :: self            !< File data.
  logical,      optional, intent(in)    :: retain_comments !< Flag for retaining eventual comments.
  integer(I4P), optional, intent(out)   :: iostat          !< IO error.
  ...
  if (present(iostat)) iostat = iostatd
  if (present(iomsg))  iomsg  = iomsgd
  endsubroutine save_t_config

  pure function section_t_config(self, section_index) result(sname)
  ...
  ! assignments
  elemental subroutine assign_t_config(lhs, rhs)
  !< Assignment between two INI files.
  class(t_config), intent(inout) :: lhs !< Left hand side.
  type(t_config),  intent(in)    :: rhs !< Rigth hand side.
  ...
  lhs%Ns = rhs%Ns
  endsubroutine assign_t_config

  ! non TBP methods
  subroutine t_config_autotest()
  !< Autotest the library functionalities.
  type(t_config)                :: fini       !< INI File.
  ...
  print "(A)", source
  call fini%free
  call fini%build(separator=':', source=source)
  print "(A)", ''
  print "(A)", "Result of parsing:"
  call fini%print(pref='  ', unit=stdout)
  ! remove "foo.ini"
  open(newunit=i, file='foo.ini') ; close(unit=i, status='DELETE')
  endsubroutine t_config_autotest
